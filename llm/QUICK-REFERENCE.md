...
- **Copart header required:** Always include the Copart header (`components/copart-header.html` for logged-out, `components/copart-header-loggedin.html` for logged-in) at the top of every prototype page unless explicitly told not to. Do not recreate or improvise a header — use the existing component files.
